...
0.1.15까지의 리셋권 조회에는 공식 사용량 조회와 다른 User-Agent를 보내 서버가 `surface` 제한으로 빈 목록을 반환하는 결함이 있었습니다. 현재 소스는 공식 사용량 조회 헤더로 수정했으며, 실제 계정에서 잔여 1개와 만료일을 읽어오는 것까지 검증했습니다. 이 소스 검증은 기존 설치 앱이 이미 교체되었다는 뜻은 아닙니다.
