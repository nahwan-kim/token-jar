...
- **네이티브 macOS 앱** — SwiftUI 기반. Dock 아이콘, 내장 브라우저, 자동 업데이트 프로그램 없이 동작합니다.
...
Gatekeeper 전체 비활성화나 quarantine 속성 제거는 필요하지 않습니다. 체크섬은 다운로드 무결성 확인용이며 개발자 신원 인증을 대신하지 않습니다. 자동 업데이트와 Homebrew 패키지는 제공하지 않으며, 업데이트는 새 ZIP으로 수동 교체합니다. 이전 버전은 교체 검증이 끝날 때까지 보관하세요.
