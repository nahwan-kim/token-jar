...
Claude는 `~/.claude/.credentials.json`의 유효한 `claudeAiOauth`만 사용하며 MCP 토큰은 사용·갱신하지 않습니다. AI 레코드가 없으면 키체인의 정확한 서비스와 `NSUserName()` 계정이 모두 일치해야 하며, 소유자 환경의 `USER`/`LOGNAME`도 `NSUserName()`에서 만듭니다. 네이티브 호출은 자동 5초·수동 120초로 제한·직렬화합니다. 소유자 터미널은 100ms quiet 뒤 구두점과 ANSI 가로 간격을 보존한 완전한 trust 질문을 확인하고 Enter로 승인하며, main banner와 shift+tab footer를 모두 확인한 뒤 `/status`를 보냅니다. 하위 프로세스는 빈 signal mask로 시작하고, 종료 요청 뒤 기존 소유자 제한 안에서 최대 1초 기다린 후 정리합니다. 재빌드한 실제 소유자의 `/status`는 약 3.17초에 완료됐지만 자연 만료 토큰 회전을 입증하지 않습니다. 정확한 계정의 폐기된 토큰은 실제 API 401이었으며 네이티브 승인 문제가 아니었습니다. 사용자가 승인한 공식 재로그인 뒤 직접 HTTP 200과 과거 v4 앱의 수동·백그라운드 fresh 사용량/계정 표시가 확인됐습니다. v4 증거는 역사적이지만, 앱 테스트 35개와 전체 UI 테스트 19개는 최신 터미널·신호·취소 수정 뒤 다시 통과했습니다. 패키지 172개/12개 스위트도 세 번 연속 통과했지만 수정 커밋의 소스 CI가 통과하기 전에는 배포할 수 없습니다. 플랜 UI와 자연 만료 changed-token 갱신은 검증되지 않았습니다. Token Jar는 토큰, 원시 응답, 원시 TUI 출력 또는 계정 식별자를 기록하지 않습니다. [CodexBar의 선호 OAuth 경로](https://github.com/steipete/CodexBar/blob/main/Sources/CodexBarCore/Providers/Claude/ClaudeOAuth/ClaudeOAuthUsageFetcher.swift)만 참고합니다.
