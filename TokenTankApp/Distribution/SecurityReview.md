...
The user delegated the source choice and approved CodexBar/Orca as references. The accepted production composition is Codex app-server, Claude Code local usage cache, Grok CLI SuperGrok credits with user-approved automatic OIDC renewal and the existing conditional bearer-only gRPC-web billing fallback, immutable Cursor.app session plus usage-summary, and official arkcli plan usage. Grok renewal applies only when the selected access token expires within 60 seconds or the first proxy request returns 401/403: after fixed issuer and selected client/scope checks, Token Jar sends a public-client `refresh_token` grant only to `https://auth.x.ai/oauth2/token`, permits no redirect or cookie, atomically preserves the new access token, expiry, and optional rotated refresh token in the same owner file, then retries the proxy once. Missing or revoked refresh credentials require `grok login`; network errors remain transient and are not classified as revocation. The CLI-proxy endpoint remains primary, and only a successful proxy snapshot with unknown usage may trigger the unchanged billing fallback with a 6-second request timeout, the same selected session bearer, no cookies, and web usage only. Proxy reset, source metadata, and account identity remain authoritative.
...
Current evidence covers the implemented renewal path without overstating it. The current source passed 117 Swift package tests, 33 app unit tests, 13 release-tool tests, the Provider I/O audit, and the forbidden FileTimestamp API check; independent integration review was clean and rotation review pass 2 was OKAY. On 2026-09-11, a token-safe current-host probe used the production `GrokOAuthSessionProvider`, forced exactly one renewal by passing the current token as `rejectedAccessToken`, then performed a real `GrokAdapter` usage fetch. It verified changed access token, rotated refresh token, returned-session/persisted-token agreement, future expiry, mode 0600, preserved noncredential metadata, successful usage with one percentage-bearing quota, and matching session/account identity; no secret was printed or copied. This does not prove a naturally expired-token event, real HTTP 401/403 injection, concurrent real CLI rotation, packaged final release, another OS, CI, or notarization. CodexBar remains a credits-behavior reference and does not directly refresh OAuth; the whole Grok authentication behavior is not CodexBar-equivalent. Browser-cookie imports, credential backups or other secret stores, Grok ACP stdio, CLI subprocess/interactive automation, manual-token UI, local estimates, xAI Management API calls, and Keychain token caches remain prohibited. Claude, Grok, Cursor, and Doubao remain `watch`; final release still requires the outstanding owner-controlled and packaged-candidate evidence, signing/notarization, clean-host verification, and AC-11 physical-host runs.

## Ownership and immutability contract

- Token Jar may write app-owned generic-password Keychain items through the Core credential capability. The sole external-owner-store exception is the user-approved Grok renewal: after a successful official grant, it may atomically replace only `~/.grok/auth.json` through a same-directory mode-0600 temporary file, without backups or another secret store.
- Other external CLI/browser stores remain immutable to Token Jar. For Grok, a bounded token-free coordination lock shared only by Token Jar processes spans the current-file reread, HTTP rotation, conflict comparison, and same-file persistence. Once renewal starts, that shared transaction settles despite observer cancellation; a canceled observer receives `CancellationError` after commit, while pre-start cancellation aborts. Comparison detects observable CLI rotation and prefers a newer session, but cannot close the final rename window against an uncooperative concurrent Grok CLI writer and is not assumed to be honored by the CLI.
- External reads use source-specific capabilities with canonical containment, no recursive discovery, and no unreviewed symlink. Write-capable operations and metadata/content mutation are forbidden except for the reviewed Grok replacement above. Provider code cannot instantiate filesystem, network, Security, process, WebKit, or browser APIs.
- Record before and after path identity, inode/file ID, size, modification time, permissions, owner, and cryptographic digest for sanitized fixtures. Read-only cases must remain unchanged; a successful Grok renewal must change only the reviewed selected-entry token/expiry fields via the atomic replacement contract. Do not use a real cookie or token for this test.
...
| Files/cache/temp | Only the Grok same-directory mode-0600 renewal temporary file, removed by atomic replacement or cleanup | Any snapshot/response/error/auth material outside the reviewed Grok temporary file; any Grok backup or other secret store | `{{FILES_CACHE_SCAN}}` |
...
- Filesystem: record canonical path, open flags, file descriptor mode, byte range/maximum, symlink decision, and close. Allow only reviewed read-only opens plus the Grok-scoped same-directory mode-0600 temporary-file and atomic-replacement operations after successful renewal. Do not use a generic home-directory scan.
...
- Process/browser/automation: Codex starts an allowlisted executable separately for each native account source, with explicit `CODEX_HOME=~/.codex` or optional `~/.codex-secondary`; the secondary uses the fixed `-c cli_auth_credentials_store="file"` override before `app-server`. Only the optional source directory is discovered, never credential files. Record resolved executable, fixed arguments, PID, deadline, and termination. Account failures and retained snapshots are isolated by source. No GJC integration, auth swapping, browser automation, or credential import is permitted.
- Mutation: hash and stat reviewed fixtures before and after. All external-owner fixtures must be byte-for-byte/field-for-field unchanged except the successful Grok renewal case, where only the selected entry's new access token, `expires_at`, and optional rotated refresh token may change through same-file atomic replacement; no backup or other secret-store entry may appear.
...
- [ ] External-owner immutability tests show no content, metadata, permission, ownership, copy, rotation, or deletion by Token Jar except the reviewed Grok same-file atomic renewal; Grok tests prove mode-0600 temporary-file replacement, selected-field-only changes, no backup/other store, and observable newer-session preference without claiming atomic safety against an uncooperative CLI writer.
...
- [ ] No quota snapshot, raw response, copied session data, or error payload persists on disk; relaunch starts `neverLoaded`. The sole provider-session persistence is the reviewed Grok same-owner-file token rotation.
- [ ] Static Provider I/O audit passes with the exact five-target scope and no suppression.
- [ ] Dynamic opens/network/mutation audit matches the matrix; symlink escape, recursive scan, and unreviewed write-capable open tests fail closed, while the exact Grok renewal replacement is recorded and bounded.
