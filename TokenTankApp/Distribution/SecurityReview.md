...
Xcode app/UI automation is excluded by explicit owner direction; release builds, signatures and remote verification remain gates. App build, actual Keychain approval/token rotation and signed-payload checks are unverified. The local GitHub CLI token is invalid and `token-jar-updates` was not found in this host's Keychain; release 0.1.13 (14) is prepared but not published, and the 0.1.12 signed feed remains unchanged.
