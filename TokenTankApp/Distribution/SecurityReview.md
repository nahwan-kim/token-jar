...
| Claude / official organization Admin usage | `https://platform.claude.com/docs/en/manage-claude/usage-cost-api` / `{{CLAUDE_DOC_REVISION}}` | every `has_more`/`next_page` page from `GET https://api.anthropic.com/v1/organizations/usage_report/messages` with current-month UTC bounds | Token Tank user owns the Admin API key | every numeric usage leaf, including nested cache-token and server-tool request categories, plus source buckets/dimensions under stable opaque IDs | injected HTTPS; app-owned Keychain credential; exact destination/query/page policy | no external path | none | no | not applicable | explicit 401/403 is app-owned authentication action; transport/Keychain failure remains stale | no external store | **SOURCE ACCEPTED AS ORGANIZATION ADMIN USAGE — never consumer Pro/Max quota; live evidence pending** |
| Grok / official xAI Management developer prepaid balance | `https://docs.x.ai/developers/management-api-guide` / `{{GROK_DOC_REVISION}}` | `GET https://management-api.x.ai/v1/billing/teams/{teamID}/prepaid/balance` | Token Tank user owns the Management key and Team ID | authoritative `total.val` USD cents and every balance change, with recursively preserved scalar metadata and stable opaque change IDs | injected HTTPS; app-owned Keychain credentials; exact destination policy | no external path | none | no | not applicable | explicit 401/403 is app-owned authentication action; transport/Keychain failure remains stale | no external store | **SOURCE ACCEPTED AS XAI MANAGEMENT DEVELOPER PREPAID BALANCE — never consumer SuperGrok quota; live evidence pending** |
...
| Doubao / official Volcano Coding Plan source | `https://docs.volcengine.com/docs/82379/1928261` / `{{DOUBAO_DOC_REVISION}}` | `GetCodingPlanUsage` at `open.volcengineapi.com`; `GetAFPUsage` remains a fixture-tested, non-composed alternative | Token Tank user owns the AK/SK pair | every Coding Plan period/window and source used/total/remaining/percentage/reset field; exact same-row remaining/percentage arithmetic only when omitted; stable opaque row IDs; no cross-action merge | injected HMAC-SHA256 HTTPS; app-owned Keychain credentials; exact destination/action/version policy | no external path | none | no | not applicable | explicit API rejection is app-owned authentication action; transport/Keychain failure remains stale | no external store | **SOURCE CLEAR — sanitized live signature/schema evidence pending** |

### Accepted source decision and remaining live gates

The user delegated the source choice and approved CodexBar as a reference. The accepted production composition is Codex app-server, explicitly labeled Claude organization Admin usage, explicitly labeled xAI Management developer prepaid balance, immutable Cursor.app session plus usage-summary, and Doubao Coding Plan OpenAPI. Live probing rejected CodexBar’s Grok ACP `x.ai/billing` path because official Grok 1.0.13 returned JSON-RPC -32601 (method-not-found). This decision does not legitimize browser-cookie imports, credential caches, Claude OAuth/PTY paths, fallback merging, or estimates. Cursor remains `watch` because its local-session layout and usage endpoint are undocumented; schema drift fails closed. Final release still requires owner-controlled live responses, terms/revision capture, actual-open immutability evidence, signing/notarization, clean-host verification, and AC-11 physical-host runs.
...
| Claude | no external location; Token Tank reads only its own generic-password item | none | app-owned Admin API credential | no System Settings request | Keychain lock remains stale; explicit API rejection requests Token Tank credential setup | no TCC/FDA grant required |
| Grok | no external location; Token Tank reads only its own generic-password items | none | app-owned Management API credentials | no System Settings request | Keychain lock remains stale; explicit API rejection requests Token Tank credential setup | no TCC/FDA grant required |
...
| Doubao | no external location; Token Tank reads only its own generic-password items | none | app-owned AK/SK | no System Settings request | Keychain lock remains stale; explicit API rejection requests Token Tank credential setup | no TCC/FDA grant required |
...
- [ ] Claude is labeled organization Admin usage everywhere and never presented as consumer Pro/Max quota.
- [ ] Grok is labeled xAI Management developer prepaid balance everywhere and never presented as consumer SuperGrok quota; the rejected ACP/browser/estimate paths are not active.
