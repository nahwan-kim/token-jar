...
| Claude / unofficial Claude Code OAuth source | pinned CodexBar `5e119a9ad76e3453058dab323f98e480f478d049` coordinator policy; OpenUsage `56378e5765f85d38ff413036fd984afe3d4664e4` read-only comparison | fixed OAuth usage/profile GETs; exact file/Keychain slots | Claude Code owns stores and `.oauth_refresh.lock` | `user:profile`; per-slot rejected-token memory (at most two) | ordinary reads strictly noninteractive; one-shot explicit repair only; no POST/write/cache | exact allowlists; empty mode-0700 repair workspace | native Keychain UI only during deliberate repair | no broad grant | dedicated localized Repair Claude Connection action | 403 no repair; authorization failure aborts; next deliberate repair required | 2026-09-16 package 206/12 ×3, app 35, bilingual UI 2, audit and universal unsigned build passed | **WATCH — real Keychain/token rotation unverified** |
...
Near expiry (60 seconds or less) or usage HTTP 401 rereads both stores noninteractively and adopts a changed valid token, including the exact-account Keychain token behind a rejected file token. The parent retries usage at most once with that changed token. HTTP 403 never repairs. Failure retains the prior in-memory snapshot as stale and exposes `RecoveryAction.repairClaudeConnection`; `keychainUnavailable` uses the same action.

Scheduled refresh, ordinary user refresh, startup, and restart never allow native prompts or launch Claude CLI. `isUserInitiated` is not repair authority. The default collection context has `allowsClaudeRecovery=false`; only `RefreshCoordinator.repairClaudeConnection()` grants one explicit one-shot capability for that collection. Concurrent repair callers coalesce. Repair pre/post-owner reloads remain noninteractive and preserve the exact account. Each deliberate repair attempts native authorization and the owner CLI at most once; authorization failure aborts without further CLI, and another button press is required. Claude Code may show its own permission UI only during that deliberate repair.
...
Current explicit-repair verification on 2026-09-16 passed package 206 tests/12 suites three consecutive runs, app 35 tests, two bilingual repair/login UI tests, the unsigned arm64+x86_64 Release build (architectures confirmed by `lipo`), Provider I/O audit, 87-key bilingual catalog/source-manifest validation, 13 release-tool tests, and `git diff --check`. Legacy Keychain deprecation and local Xcode device-framework warnings were not suppressed; macOS build/tests succeeded. The 2026-09-12 counts remain historical automatic-recovery evidence only. No real Keychain authorization, authenticated token rotation, installation, or deployment was performed for this change.
...
- Token Jar accepts only Claude AI OAuth credentials, never MCP tokens. It neither changes Claude credentials nor item ACLs/user Keychain settings, sends a refresh POST, nor creates a token cache. Scheduled and ordinary manual collection are read-only and cannot launch Claude CLI; only one deliberate Repair Claude Connection action may let Claude Code rotate its own AI OAuth store under `.oauth_refresh.lock`.
...
The external `Claude Code-credentials` item is read-only to Token Jar and is queried only with `kSecAttrAccount=NSUserName()`; service-only lookup is forbidden because it selected another OS account during QA. Ordinary scheduled/manual/startup/restart reads and repair pre/post-owner reloads are noninteractive, bounded, serialized, and preserve the exact account. Native authorization is available only to the dedicated one-shot repair capability, at most once per repair. It changes no ACL or user Keychain setting. Authorization failure aborts before any further CLI launch. Because the no-UI setting is process-local, Claude Code may still show its own permission UI during deliberate repair; no such child is launched during ordinary collection. Deprecated Keychain APIs and UI Allow/Fail query flags remain intentionally combined with `LAContext` for legacy behavior; do not claim warning-free compilation.
...
| Native Claude query | exact service-and-`NSUserName()` account lookup; ordinary reads and repair pre/post-owner reloads noninteractive; dedicated repair permits at most one native authorization attempt; timeout/cancel discards late data | current injected-native/session tests pass; real Keychain authorization remains unverified |
...
- Filesystem: allow exact read-only provider paths, the deliberate Claude repair's empty mode-0700 temporary workdir removed after bounded cleanup, and the Grok mode-0600 atomic-replacement temporary file. The Claude workdir must contain no credential, snapshot, response, error, or raw TUI persistence. Do not scan a home directory.
...
- Keychain: accept only `claudeAiOauth`; MCP is forbidden. Query only the exact service plus `kSecAttrAccount=NSUserName()`. Scheduled and ordinary manual reads plus repair pre/post-owner reloads are noninteractive. Only the one-shot explicit repair may make at most one native authorization attempt; failure aborts. Do not accept late results or change ACLs/user settings.
- Process/browser/automation: no ordinary collection launches Claude CLI. One deliberate repair may launch at most one fixed no-tools owner `/usage` process; prompt veto precedes readiness/success, trust is accepted only in the verified-empty workspace and never after `/usage`, and authorization failure launches no replacement. Claude Code may show its own permission UI only in this explicit repair. Browser automation, cookie import, login approval, and PTY usage scraping remain forbidden.
- Mutation: hash and stat reviewed fixtures before and after. Token Jar must not POST, write, copy, or cache Claude tokens. Claude Code alone may rotate its store under `.oauth_refresh.lock` during deliberate repair; Token Jar rereads noninteractively and requires a changed fresh token. The only direct external-store mutation by Token Jar is the reviewed Grok selected-field atomic replacement.
...
| Claude explicit repair | exact source slots; one-shot scoped repair authority; at most one native authorization and one bounded owner launch; noninteractive pre/post reloads | Claude Code owns rotation/lock; Token Jar performs no token POST/write/copy/cache | synthetic denial/cancellation/rotation and authority tests pass in package 206/12 ×3; real Keychain/authenticated rotation unverified |
...
- General permission denial or revocation remains stale and never triggers a workaround. For Claude, unavailable authorization maps to `.repairClaudeConnection`, preserves the in-memory value, and launches no additional CLI. Ordinary cycles, manual refresh, and restart do not grant retry permission; the user must deliberately invoke repair again.
...
| Claude | exact file/service/OAuth/executable | native Keychain UI only during deliberate repair; no TCC/FDA | exact read-only credentials; all ordinary and repair pre/post-owner reads are noninteractive; one-shot repair alone may launch one owner CLI | dedicated localized Repair Claude Connection action; Claude Code may show its own permission UI only then | 403 does not repair; authorization failure aborts; missing/revoked requires `claude auth login`; ordinary refresh/restart gives no retry permission | no direct token POST/write/copy/cache or ACL/user-setting mutation; WATCH |
...
- [ ] Scheduled, ordinary manual, startup, restart, and repair pre/post-owner Claude reads are noninteractive; `isUserInitiated` is not repair authority and the default collection context has `allowsClaudeRecovery=false`.
- [ ] Only the dedicated localized Repair Claude Connection action invokes `RefreshCoordinator.repairClaudeConnection()` with one-shot scoped authority; concurrent callers coalesce, each repair attempts native authorization and owner CLI at most once, failures abort, and the next attempt requires another deliberate repair.
...
- [ ] Dynamic audits cover near-expiry and 401 noninteractive reread, changed-token-only single HTTP retry, 403 no-repair, stale retention with typed `.repairClaudeConnection` including `keychainUnavailable`, no CLI on ordinary refresh/restart, explicit-repair coalescing, one-shot authority, at-most-one native prompt/owner launch, authorization-failure abort, and forbidden fallbacks. Real Keychain authorization and authenticated token rotation remain live-verification gaps.
