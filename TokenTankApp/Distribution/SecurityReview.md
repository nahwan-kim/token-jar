...
- Network: record the injected request destination, method, header classes without values, timeout, cancellation, status, and body size. Claude is limited to OAuth `GET /api/oauth/usage`, optional same-token `GET /api/oauth/profile`, and optional same-token `GET /api/oauth/usage?cedar_ember=1&skip_spend=1` at `api.anthropic.com`. The supplemental reset query permits only that exact query string, no request body, and at most 15 seconds; arbitrary, duplicate, encoded, or profile query parameters remain forbidden. Its `cedar_ember.grants` quantity/expiration schema comes from integrity-verified official Claude Code 2.1.280 native source, not a live-account proof. Only primary usage may retry once with a changed valid token; supplemental errors do not authorize recovery. HTTP 403 does not repair. No reset-redemption, inference, or Admin API request.
