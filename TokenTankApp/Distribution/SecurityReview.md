...
|| Grok / Grok CLI SuperGrok credits | `https://github.com/steipete/CodexBar/blob/main/docs/grok.md` / current | Primary `GET https://cli-chat-proxy.grok.com/v1/billing?format=credits` after a read-only `~/.grok/auth.json` open; only when a successful proxy response leaves usage unknown, `POST https://grok.com/grok_api_v2.GrokBuildBilling/GetGrokCreditsConfig` | Grok CLI owns and rotates the session | primary published `creditUsagePercent` or same-row on-demand used/cap ratio, plus proxy reset/source metadata and account identity; fallback published percentage, or omitted proto3 scalar as zero only for a structurally complete response with a recognized current weekly/monthly period containing the refresh time | exact external-session read of `~/.grok/auth.json` plus injected HTTPS GET/conditional POST; same captured bearer only, empty 5-byte gRPC-web data frame (`00 00 00 00 00`), no cookies, 6-second timeout; token exists only in the ephemeral request | canonical owner path | none identified | no | not applicable | missing/expired/rejected owner session requests `grok login`; a successful proxy with unknown usage may trigger one fallback; failed retry or unrecognized/malformed fallback retains unknown usage and proxy metadata; no extra retry chain | no mutation | **WATCH — CodexBar-style bearer-only fallback authorized; primary/fallback endpoint behavior and parser are not live-validated; fixture tests planned** |
...
The user delegated the source choice and approved CodexBar/Orca as a reference. The accepted production composition is Codex app-server, Claude Code local usage cache, Grok CLI SuperGrok credits with a conditional bearer-only gRPC-web billing fallback, immutable Cursor.app session plus usage-summary, and official arkcli plan usage. The Grok CLI-proxy endpoint remains primary; only a successful proxy snapshot with unknown usage may trigger one fallback with a 6-second request timeout using the same captured bearer, no cookies, and web usage only. This is the transport request timeout, not a separately enforced whole-operation deadline. Proxy reset, source metadata, and account identity remain authoritative. On 2026-09-07, the production Swift adapter and network capability recovered used 0% and remaining 100% from a validated active-period proto3 implicit zero after a period-only proxy response; the protocol does not distinguish an omitted zero from a withheld scalar, so this matches CodexBar interpretation rather than an independently published percentage. All 97 package tests, including fallback and network-boundary fixtures, and the Provider I/O audit passed. Packaged-app and final frozen-candidate verification remain pending. Earlier live probing rejected CodexBar’s Grok ACP `x.ai/billing` path because official Grok 1.0.13 returned JSON-RPC -32601 (method-not-found). This decision does not legitimize browser-cookie imports, credential caches, Grok ACP stdio, Claude OAuth/PTY paths, local estimates, xAI Management API calls, credential refresh/copy, or arbitrary fallback merging. Claude, Grok, Cursor, and Doubao remain `watch` because their local-session or CLI JSON layouts are undocumented; schema drift fails closed. Final release still requires owner-controlled live responses, terms/revision capture, actual-open immutability evidence, signing/notarization, clean-host verification, and AC-11 physical-host runs.
...
|| Grok | `~/.grok/auth.json`; primary `cli-chat-proxy.grok.com/v1/billing?format=credits`; conditional `grok.com/grok_api_v2.GrokBuildBilling/GetGrokCreditsConfig` | none | Grok CLI owns credential access; Token Jar reads only the owner session, keeps the proxy primary, and may make one same-bearer gRPC-web fallback with no cookies | no System Settings request | missing/expired/rejected owner session requests `grok login`; unknown proxy usage and failed/unrecognized fallback remain unknown with proxy metadata preserved; no credential refresh/copy or extra fallback | no TCC/FDA grant required |
...
- [ ] Grok reads only the read-only Grok CLI owner session plus primary CLI-proxy credits; only a successful proxy response with unknown usage may trigger one same-bearer, no-cookie, 6-second gRPC-web billing fallback, which supplies usage only and preserves proxy reset/source metadata; it never imports cookies, uses Grok ACP stdio, estimates locally, calls the xAI Management prepaid-balance API, or refreshes/copies credentials.
