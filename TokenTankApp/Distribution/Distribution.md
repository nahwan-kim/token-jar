...
Promotional reset tickets use one additional optional same-token `GET https://api.anthropic.com/api/oauth/usage?cedar_ember=1&skip_spend=1` (15-second maximum). The ordinary usage request remains unchanged. The exact query and `cedar_ember.grants[]` contract are evidenced by the published native `@anthropic-ai/claude-code-darwin-arm64@2.1.280` package, whose npm SHA-512 integrity was verified: `ctkNgja8Yi2kngVFPO2667k6zbtJwjQ+dOTeEp1XmzHcoDFdaee4h4WVgZllsewm/Io+pPPPSFQVdGHOtdE/1A==`. Each grant supplies `id`, `resets_left`, and optional ISO `starts_at` / `ends_at`. The UI preserves the source sum of `resets_left` rather than recomputing ownership from local dates. It shows the nearest future expiration among positive-count grants below the five-hour gauge, alongside Fable under the weekly gauge. `paused`, `usable_now`, and `eligible` do not erase explicit grant counts. Missing or malformed metadata and empty grants without `eligible:true` remain unknown; only explicitly eligible empty grants or explicit zero counts prove zero. Supplemental failures never discard valid primary usage or trigger authentication recovery. Cancellation still propagates. No reset-redemption POST is permitted. Live account validation remains pending: the repeated noninteractive probe had no usable session and Keychain returned `-25293`; no authenticated HTTP response was obtained, so the reported live zero's precise cause is unconfirmed.
