...
For v0.1.10, the user explicitly accepted publication with natural-expiry Claude `/status` token rotation, real upgrade, physical Intel, and clean/quarantined-host checks disclosed as unverified. This acceptance does not waive CI or security failures. Source revision `489ba51` failed both macOS CI jobs on owner `/exit` teardown; no release, draft, or feed was published. Source CI must pass on the repair commit before publication, followed by clean-revision packaging and the mandatory signature/checksum/remote gates.
...
Locked/off-console Keychain calls remain serialized and bounded at 5 seconds background/120 seconds manual. Owner renewal uses the reviewed no-tools process with canonical `NSUserName()` identity. Current parsing preserves question punctuation and ANSI horizontal spacing, waits for 100 ms quiet, requires the complete exact trust question plus Enter, then requires the main banner and shift+tab footer before `/status`. The child starts with an empty signal mask and, after exit is written, may run up to 1 second within the owner deadline before bounded teardown. A credential-free probe confirmed inherited SIGALRM blocked=true and corrected child state=false; a rebuilt real-owner `/status` completed in about 3.17 seconds. Neither proves natural-expiry token rotation. Historical v4 live HTTP 200/manual/background recovery remains valid for its revision only and predates these terminal repairs. No new login or Keychain authorization blocker exists.
...
| AC-03 | Current native-session 28 and owner-refresher 15 passed; package 172/12 passed three consecutive runs; post-repair app 35/full UI 19 passed. | Repair-commit source CI; natural-expiry rotation and final clean-release verification. |
...
| AC-07 | Owner parsing now requires 100 ms quiet, complete trust question + Enter, banner/footer landmarks, empty child signal mask, and bounded 1 s exit wait. Real rebuilt `/status` completed ~3.17 s. | Repair-commit CI; `/status` is not expiry-rotation proof. |
| AC-08 | Exact AI/account rules remain unchanged. Native-session 28 and post-repair app 35/UI 19 passed. | Final repair candidate and CI; never broaden Keychain matching. |
| AC-09 | Historical v4 evidence after the USER/LOGNAME and exact-account fixes verified official re-login, exact-account HTTP 200, manual fresh usage/account, and same-binary background cold start without a native prompt. It predates current terminal/cancellation repairs and is not final-artifact evidence. | Natural-expiry renewal and plan UI remain unverified; `/status` and re-login do not prove expiry rotation. |
...
| AC-12 | Full UI 19 reran and passed after terminal, signal, and cancellation repairs. | Repair-commit CI and clean-artifact rerun remain pending. |
| AC-13 | Full 171 exposed cancellation before waiter registration; the deterministic regression failed. Moving the check inside the installed cancellation handler sealed the gate, after which package 172/12 passed three consecutive runs. No release, draft, or feed was published. | Repair-commit source CI, then clean rebuild/package and publication gates. |
...
| AC-15 | Current repair candidate preserves exact account/environment and 5 s background/120 s manual bounds, adds strict terminal landmarks, empty spawn signal mask, bounded exit wait, and cancellation gate sealing. Native 28, owner 15, and package 172/12 ×3 passed locally. | Repair-commit source CI and clean rebuilt-artifact security; historical v4 is not publishable. |
