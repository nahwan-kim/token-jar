...
All five current adapters use credentials owned by their provider tools and do not request app-owned credential setup. Claude reads only the exact native Claude Code file and generic-password Keychain account and never copies, stores, writes, directly refreshes, or POSTs its token. Scheduled and ordinary user refreshes are strictly noninteractive reads and never launch Claude CLI. Near expiry or usage HTTP 401 may reread the stores and adopt a changed valid external token; only the dedicated explicit repair action may run one bounded Claude Code owner `/usage` flow, under Claude Code's official `.oauth_refresh.lock`. The only direct owner-store mutation by Token Jar is the user-approved Grok OIDC renewal described below. The Core data-protection Keychain for app-owned credentials remains a separate fail-closed facility; ad-hoc distribution does not promise it works, and it is not a fallback for Claude or any other owner session.
...
## Claude OAuth usage and explicit connection repair
...
Only a successful live usage response is fresh. Near expiry (60 seconds or less) or usage HTTP 401 causes a noninteractive reread of both exact stores. A changed valid external token is adopted and the parent retries usage at most once, only with that changed token. HTTP 403 does not repair or renew. Network, timeout, schema, authorization, 403, 429, or failed repair retains a prior in-memory result only as stale; there is no stale-cache-as-fresh or `~/.claude.json` fallback. `keychainUnavailable` and exhausted authentication rereads expose `RecoveryAction.repairClaudeConnection`.

Scheduled refreshes, ordinary user refreshes, startup, and restart are strictly noninteractive and never launch Claude CLI or permit native Keychain UI. `isUserInitiated` does not confer repair authority. Only the dedicated localized **Repair Claude Connection / Claude 연결 복구** action grants `RefreshCoordinator.repairClaudeConnection()` one one-shot capability scoped to that collection; the default collection context has `allowsClaudeRecovery=false`, while this explicit path alone sets it true. Concurrent repair callers coalesce. General manual refresh is not repair, and an ordinary refresh or restart never grants implicit retry permission.

One deliberate repair performs noninteractive pre-owner and post-owner reloads, preserving the exact account. It may attempt native authorization at most once and may launch at most one bounded owner process. Native authorization failure aborts without launching or relaunching the CLI; another deliberate repair is required. Claude Code may display its own permission UI only during this explicit repair. The owner process uses fixed absolute executable candidates, an empty mode-0700 workspace, a minimal fixed environment, fixed safe-mode/no-tools arguments, and the existing bounded owner timeout. Prompt veto precedes readiness and success detection; only the exact trust question may be accepted in the verified-empty workspace, and never after `/usage`. Authorization or other prompt failure aborts rather than launching more CLI.
...
Current explicit-repair verification on 2026-09-16: package 206 tests/12 suites passed three consecutive runs using an alternate scratch directory; app 35 tests and two bilingual repair/login UI tests passed; the unsigned universal arm64+x86_64 Release build succeeded, with both architectures confirmed by `lipo`. Provider I/O audit, 87-key bilingual catalog/source-manifest validation, release-tool tests (13), and `git diff --check` passed. Existing legacy Keychain deprecation warnings and local Xcode CoreDevice/CoreSimulator environment warnings remain visible; macOS build/tests succeeded. No real Keychain authorization or authenticated token rotation was validated; no installation or deployment occurred. Historical 2026-09-12 counts (194 package tests/12 suites) concern the superseded automatic-recovery revision, not the current implementation.
...
| AC-03 | Current 2026-09-16 package 206 tests/12 suites passed three consecutive runs for explicit repair. | Natural-expiry/real-401 authenticated rotation and final clean-release verification. |
| AC-04 | Current app 35 tests and two bilingual repair/login UI cases passed. | Final signed-candidate bilingual smoke test. |
| AC-05 | Current fake-clock/gated tests cover silent scheduled/manual collection, scoped one-shot authority, repair coalescing, cancellation, and stop/restart. | Final-candidate live cadence check. |
| AC-06 | Current Coordinator/Adapter/AppModel tests cover typed repair failures, retained last-good usage, cancellation cleanup, and no authority leakage to later manual refresh. | Final-candidate live denial/relaunch check. |
| AC-07 | Current synthetic session tests cover at-most-one native authorization and owner launch, denial abort, unchanged-token failure, and shared repair cancellation. | Live authenticated changed-token recovery and real Keychain authorization. |
| AC-08 | Current exact-account tests and repair interaction-flag assertions pass; pre/post-owner reads are noninteractive. Intentional legacy Keychain warnings remain. | Supported-OS native Keychain behavior; child UI is permitted only during explicit repair, not claimed to be suppressed. |
| AC-09 | Current source-level explicit-repair tests/build pass; historical v4 HTTP 200 remains revision-scoped. | Natural-expiry, real-401 changed-token recovery, and real Keychain authorization remain unverified. |
| AC-10 | Historical v4 and clean `489ba51` artifacts predate current terminal/cancellation repairs; all v1-v4 and `489ba51` ZIPs are superseded and nonpublishable. | Rebuild only after repair-commit CI passes. |
...
| AC-12 | Current package 206/12 ×3, app 35, bilingual UI 2, audit, manifest/catalog validation, release-tool 13 tests, and universal unsigned Release build passed. | Clean signed-artifact rerun and publication CI. |
...
| AC-15 | Current explicit-repair tests, Provider I/O audit, manifest/localization validation, and universal unsigned Release build passed. Intentional legacy Keychain warnings remain. | Real Keychain/token-rotation validation, signed/notarized packaging, installation, and clean rebuilt-artifact security. |
