...
This exclusion does not waive the universal Release build, source CI, payload/secret audit, code signatures, Sparkle update signatures, checksum validation, or remote feed verification. The development host has Command Line Tools but no full Xcode; build the clean revision on an Xcode-capable release/CI host. GitHub publishing requires valid operator authentication. The existing `token-jar-updates` private key remains in the release operator's login Keychain: never export or replace it to work around a missing key on this host.
...
Release candidate identity is 0.1.13 (14), not a published release. App/XCUITest automation is excluded by owner request. This host has no Xcode, its GitHub CLI token is invalid, and its Keychain lookup found no `token-jar-updates` signing account. Universal app build, real native approval/token rotation, signed ZIP/payload audit, installation and publication remain outstanding. Keep the existing 0.1.12 feed unchanged until the normal release gates succeed.
