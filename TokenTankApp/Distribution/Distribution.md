...
All five current adapters use credentials owned by their provider tools and do not request app-owned credential setup. Claude reads only the exact native Claude Code file and generic-password Keychain account and never copies, stores, writes, directly refreshes, or POSTs its token. Near expiry or usage HTTP 401 may automatically invoke a bounded Claude Code owner `/usage` flow; Claude Code alone owns rotation and its official `.oauth_refresh.lock`. The only direct owner-store mutation by Token Jar is the user-approved Grok OIDC renewal described below. The Core data-protection Keychain for app-owned credentials remains a separate fail-closed facility; ad-hoc distribution does not promise it works, and it is not a fallback for Claude or any other owner session.
...
## Claude OAuth usage and automatic recovery

Token Jar accepts only `claudeAiOauth` from the exact native file or generic-password service `Claude Code-credentials` item with `kSecAttrAccount=NSUserName()`. Rejected-token memory is retained independently for the file and Keychain slots (at most two) even when the other store supplies a fresh token, preventing A/B oscillation; if both sources are rejected, the next eligible 401 forces owner renewal. MCP tokens are never accepted or renewed. A usable token requires `user:profile`, finite positive expiry, and more than 60 seconds remaining. Expiry is checked after each potentially slow read.

Only a successful live usage response is fresh. Near expiry or usage HTTP 401 causes a credential reread and automatic recovery; HTTP 403 does not renew. The parent retries usage at most once and only with a changed valid token. Network, timeout, schema, 403, 429, or failed recovery retains a prior in-memory result only as stale; there is no stale-cache-as-fresh or `~/.claude.json` fallback. Token Jar does not use browser cookies, scrape usage from PTY output, call an Admin API or LLM inference endpoint, automate login/browser approval, or create an app-owned Claude credential/usage cache.

`allowInteraction` controls only native Keychain UI. Background initial reads and reloads remain noninteractive; user-initiated reads may allow native Keychain UI. Native calls remain serialized and bounded at 5 seconds background/120 seconds interactive, and locked/off-console paths skip them. `SecKeychainSetUserInteractionAllowed(false)` is process-local, so Token Jar cannot guarantee that an independent Claude Code or security child never displays macOS authorization UI; no ACL or user Keychain setting is changed.

Before renewal Token Jar rereads both sources and adopts a changed valid token, including an exact-account Keychain token behind a rejected file token. Otherwise one shared owner attempt runs only while unlocked/on-console. A failed token enters a token-keyed 300-second background cooldown; callers still reread credentials first, a fresh external token is adopted immediately, and an explicit manual refresh bypasses the cooldown. The owner timeout is 60 seconds so Claude Code can perform its official locking and renewal. Token Jar never sends a refresh POST or writes a Claude store; Claude Code owns rotation and `.oauth_refresh.lock`.

The owner process uses fixed absolute executable candidates, an empty mode-0700 workspace, a minimal fixed environment, and fixed safe-mode/no-tools arguments. Prompt veto precedes readiness and success detection; only the exact trust question may be accepted in the verified-empty workspace, and never after `/usage`. After readiness banner plus shift+tab it sends exactly `/usage`; loading or tab headers do not complete, and a recognized completed panel must remain quiet for at least one second. Completion sends Esc and `/exit` and allows five seconds for natural exit. A hard deadline after `/usage` performs a passive five-second wait with no input before bounded termination; a pre-readiness deadline performs immediate cleanup without input.

Missing or revoked credentials require `claude auth login`; a web or Claude Desktop login is not a substitute, and Token Jar never automates login or browser approval. Inspection of Claude Code 2.1.234 embedded JavaScript found the `/usage` forced-renewal path using the official lock. A hand-driven no-tools probe reached the Usage panel while `claude auth status` reported `loggedIn:false`. The final rebuilt production runner safely stopped at the login-required marker in 0.57 seconds with static diagnostic `claude.auth-refresh.interaction-rejected.login` and `.signInSourceApp`. Neither probe establishes authenticated token rotation. Natural-expiry and real-401 changed-token recovery remain unverified; historical v4 evidence applies only to its revision.

Local verification on 2026-09-12 used clean alternate scratch because stale `.build` referred to another checkout. Package tests passed all 194 tests in 12 suites on three consecutive final runs. App 35 and the bilingual Claude recovery UI case passed against final source, and the unsigned universal arm64+x86_64 Release rebuild succeeded. One unrelated Codex `repeatedFastProcessExit` one-second timeout occurred earlier; isolated reproduction and three subsequent whole-suite runs passed without Codex changes. Provider I/O six-file audit and manifest/localization validation passed. Independent Grok review approved the session, owner runner, adapter and native/PTY tests with no findings; this is not live-rotation or release sign-off. No installation or deployment occurred, and intentional legacy Keychain deprecation warnings remain.
...
| AC-03 | Final clean-scratch package runs passed 194 tests/12 suites three consecutive times, including per-source rejected-token retention, A/B oscillation prevention, and next-eligible-401 owner renewal when both slots are rejected. | Natural-expiry/real-401 authenticated rotation, source CI, and final clean-release verification. |
| AC-04 | App 35 and the bilingual Claude recovery UI case passed against final source. | Final signed-candidate bilingual smoke test. |
...
| AC-07 | Package 194/12 PASS ×3 covers prompt veto ordering, no trust after `/usage`, passive post-usage deadline wait, and immediate pre-readiness cleanup. Production login-required handling failed closed; independent implementation review approved. | Live authenticated changed-token recovery. |
| AC-08 | Final package 194/12 PASS ×3 and app 35 PASS cover exact slots, retained per-source rejection memory, and noninteractive reloads. Intentional legacy Keychain warnings remain. | Final source CI; never claim child UI suppression or a warning-free build. |
| AC-09 | Historical v4 evidence verified exact-account HTTP 200 and manual/background fresh usage for its revision. The Claude Code 2.1.234 source inspection found official `/usage` locking, and a no-tools live command reached the Usage panel while `claude auth status` was `loggedIn:false`. | Natural-expiry and real-401 changed-token recovery remain unverified; historical evidence and panel reachability do not prove this implementation. |
...
| AC-12 | Package 194 tests/12 suites PASS ×3; app 35 and bilingual Claude recovery UI 1 PASS; universal unsigned Release rebuild succeeded. An earlier unrelated Codex timeout passed isolated reproduction and three subsequent whole suites without Codex changes. | Source CI and clean signed-artifact rerun. |
...
| AC-15 | Provider I/O audit passed six files; manifest/localization validation passed; unsigned universal Release build succeeded; independent Claude implementation review approved. Intentional legacy Keychain warnings remain. | Authenticated renewal, source CI, signed/notarized packaging, installation, and clean rebuilt-artifact security; unsigned build success is not deployment evidence. |
