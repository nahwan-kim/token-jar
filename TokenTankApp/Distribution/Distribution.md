...
For v0.1.10, the user also explicitly accepted publication with natural-expiry Claude `/status` token rotation unverified. Official interactive re-login and subsequent live/manual/background usage recovery were verified; they do not prove automatic expiry renewal. Include this limitation, the unperformed real upgrade, and physical Intel/clean-host checks in the release notes. All CI, clean-revision packaging, signature, checksum, and remote-publication gates still apply.
...
All five current adapters use credentials owned by their provider tools and do not request app-owned credential setup. Claude reads the exact native Claude Code file or generic-password Keychain item but never copies, stores, writes, or directly refreshes it; only explicit manual Refresh can permit a native Keychain prompt and bounded Claude Code owner renewal. The only direct owner-store mutation by Token Jar is the user-approved Grok OIDC renewal described below. The Core data-protection Keychain for app-owned credentials remains a separate fail-closed facility; ad-hoc distribution does not promise it works, and it is not a fallback for Claude or any other owner session.
...
## Claude OAuth usage and manual recovery

Token Jar accepts only `claudeAiOauth`. A valid file AI record is authoritative; if AI OAuth is absent, including a valid MCP-only file, only the native generic-password service `Claude Code-credentials` item with `kSecAttrAccount=NSUserName()` may be tried. Service-only lookup selected another OS account during QA and is forbidden. A present malformed, wrong-scope, or invalid-expiry AI record fails without fallback. MCP tokens are never accepted or renewed; MCP-only in both stores maps to missing/source-login. Expiry is checked after each potentially slow read.

Only a successful live usage response is fresh. HTTP 401/403 is an authentication failure, 429 observes backoff, and network, timeout, or schema errors retain only the prior in-memory result as stale. There is no `~/.claude.json` usage-cache fallback. Token Jar does not use browser cookies, scrape usage from PTY output, call an Admin API or LLM inference endpoint, or create an app-owned Claude credential/usage cache.

Locked/off-console paths skip Keychain. Other native calls are serialized and bounded at 5 seconds in background and 120 seconds for explicit manual **Refresh**, allowing time for human password or biometric entry; timeout/cancel discards a late result and proves only that the bound elapsed, not that approval was declined. Background access temporarily disables the process-local interaction policy and restores it on every exit, while neither mode changes ACLs or user settings. After expiry, owner renewal uses exact `--safe-mode --tools '' --allowed-tools '' --setting-sources '' --strict-mcp-config --mcp-config '{"mcpServers":{}}'`. The owner `safeEnvironment` supplies `USER` and `LOGNAME` from `NSUserName()` rather than inheriting arbitrary values; omitting those variables caused the earlier false login blocker. The corrected production `/status` touch completed in about 1.28 seconds, but a status touch is not evidence that `/status` automatically rotates a naturally expired token. The correct Keychain account's token was read successfully; live API 401 `authentication_error` responses with a revoked indication under User-Agent versions 2.1.0 and 2.1.234 proved provider authentication failure rather than missing native approval. The user then explicitly authorized official `claude --safe-mode auth login --claudeai`, which exited 0 after browser approval. An exact-account direct OAuth usage GET subsequently returned 200 with numeric five-hour and seven-day utilization, reset strings, limits, and extra-usage presence. Frozen dirty-source local-v4 manual Refresh showed fresh Claude usage percentage and account label; restarting the same approved binary produced fresh background cold-start usage and account with no native prompt observed. Official interactive re-login recovery is therefore verified. Natural-expiry changed-token renewal remains unverified, and the plan UI was not verified because its accessibility identifier was absent. The runner accepts only the two reviewed exact empty-workspace trust questions, denies unknown prompts, and never logs tokens, raw API bodies, raw values, account identities, or raw TUI output.
...
| AC-03 | Latest full package suite passed 168 tests in 12 suites, the native total is 27, the post-fix app suite passed 35 tests, the post-fix full UI suite passed 19 tests, and the universal Release build succeeded. | Natural-expiry changed-token renewal and final clean-revision release verification. |
...
| AC-07 | Claude native calls use one bounded worker/reader plus process-wide serialization; background is bounded at 5 seconds and manual at 120 seconds, unresolved timeouts block new queries, late data is discarded, and background interaction policy is always restored. Token Jar changes no ACL/user setting; only Claude Code may rotate AI OAuth during manual renewal. Frozen v4 manual and same-binary no-prompt background collection succeeded. | Verify natural-expiry changed-token renewal and the final clean candidate; a timeout alone is not approval-denial evidence. |
| AC-08 | Only `claudeAiOauth` is accepted. Missing AI may try only Keychain service `Claude Code-credentials` plus `kSecAttrAccount=NSUserName()`; present invalid AI cannot fall back; MCP is never used/renewed; both-store MCP-only maps missing/source-login. The 27-test native suite covers same-service account isolation and no broadening when the current account is absent. | Verify the final clean candidate and expiry-after-read behavior; never restore service-only matching. |
| AC-09 | The old Core environment omitted `USER`/`LOGNAME`; production now supplies both from `NSUserName()`. Native Keychain lookup also pins `kSecAttrAccount=NSUserName()` because service-only lookup selected another account. The correct-account revoked-token 401 was provider authentication failure, not missing native approval. User-authorized official interactive re-login exited 0 after browser approval, followed by exact-account HTTP 200 usage, frozen v4 manual fresh usage/account, and same-binary background cold-start fresh usage/account without an observed native prompt. | Natural-expiry changed-token renewal remains unverified; `/status` touch and interactive re-login do not prove it. Plan UI is also unverified because its accessibility identifier was absent. |
| AC-10 | Release settings, `LSUIElement`, exact dependency/payload/import/leak scans, and UI termination tests enforce no Dock/browser UI/unreviewed helpers and a stripped universal artifact. Sparkle is the explicit signed-update exception. Independent v4 verification confirmed graceful app termination. | Repeat artifact checks against the final clean-source release. |
...
| AC-12 | The deterministic XCUITest matrix exposes all five groups plus fresh, stale, authentication, permission, offline, missing, zero, direction, reset/freshness, and recovery controls. The complete post-fix UI suite passed 19 tests. | Repeat against the final clean-source release candidate. |
| AC-13 | Universal v0.1.10/build 11 dirty-source local-v4 packaged successfully at `/tmp/token-jar-0.1.10-localqa-v4/TokenJar-0.1.10.zip` with SHA-256 `d8b1c4512975c2843971e463ce13f28674deaf45327b47609434c1fe4849a167`. Independent verification confirmed the feed and archive Ed25519 signatures, archive length and SHA-256, and graceful app termination. Local-v1 through local-v3 are obsolete. No dirty-source QA artifact may be published. | Commit, rebuild, package, and sign from the final clean release revision; complete real upgrade, physical Intel, clean/quarantined-host, signing, and publication checks. |
...
| AC-15 | Claude uses exact AI OAuth inputs, exact Keychain service plus `kSecAttrAccount=NSUserName()`, canonical `NSUserName()` owner environment, 5-second background and 120-second manual serialized bounds, restored process-local background interaction policy, and no ACL/user-setting mutation; MCP credentials are forbidden. Live exact-account usage, official interactive re-login recovery, frozen v4 manual refresh, and same-binary background cold start are verified. | Verify natural-expiry changed-token renewal and rebuilt clean-artifact privacy/security; do not infer renewal from `/status` touch or re-login. |
