...
### QA scope for the Command Line Tools development host

As explicitly requested by the owner on 2026-09-18, default local and CI QA excludes Xcode application-unit and UI automated test execution. Retain the test sources; report these checks as excluded, never passed. Package regressions (three runs before release), Swift syntax checks, localization/source-manifest/plist validation, Provider I/O audit, release-tool tests, and change review remain required. Real application/authentication checks are separate evidence and must disclose unavailable coverage.

This exclusion does not waive the universal Release build, source CI, payload/secret audit, code signatures, Sparkle update signatures, checksum validation, or remote feed verification. The lab development host has Command Line Tools but no full Xcode; use an Xcode-capable build host for the exact clean revision. As explicitly authorized on 2026-09-18, lab is now the release-signing host and holds the existing `token-jar-updates` key in its login Keychain. The master retains the original as a backup. This is a one-time release-key migration, not part of shared configuration synchronization. Never generate a replacement signing key or weaken Keychain access to work around SSH-session restrictions.
...
Historical record for source revision `489ba51`: the user accepted v0.1.10 publication with natural-expiry Claude rotation, real upgrade, physical Intel, and clean/quarantined-host checks disclosed as unverified, but both macOS CI jobs failed on owner `/exit` teardown and no release, draft, or feed was published for that revision. Its `/status` work did not prove token rotation and does not verify the current `/usage` recovery implementation. This historical acceptance did not waive CI or security failures.
...
The 2026-09-18 migration copied the existing key from the master's login Keychain to lab's login Keychain through authenticated SSH and a user-only FIFO (no secret bytes persisted in a file). Sparkle 2.9.6's official importer ran in lab's logged-in Terminal GUI session and its public-key readback matched the pinned `SUPublicEDKey`. The transient FIFO was removed and the original key preserved. No private key was placed in Git, command arguments, logs, release assets, or configuration-sync payloads.

On these hosts, direct SSH/headless access to this sensitive Keychain item is rejected by macOS. Run packaging in lab's logged-in Terminal GUI session and approve the native requests for the verified `generate_keys`, `sign_update`, and `generate_appcast` tools as needed. Do not unlock the Keychain with a password in a command, grant unrestricted application access, export a plaintext key file, or alter the trust root. Pinned Sparkle tooling and its archive are available at `~/.local/share/token-jar/sparkle/2.9.6` on lab; packaging still verifies the archive checksum before using it. The signed app and generated feed must pass the usual independent audits before publication.
...
All five current adapters use credentials owned by their provider tools and do not request app-owned credential setup. Claude reads only the exact native Claude Code file and generic-password Keychain account and never copies, stores, writes, directly refreshes, or POSTs its token. Every Claude collection, including the default five-minute schedule and ordinary manual refresh, first rereads those stores noninteractively. If recovery is required, that collection has at most one opportunity to pass the screen-unlocked guard, request native Keychain authorization once, and, for expired or rejected credentials, run one bounded Claude Code owner `/usage` flow under Claude Code's official `.oauth_refresh.lock`. Denial aborts the current attempt without another CLI or prompt loop; a later scheduled collection may try again. The dedicated repair action is an immediate manual retry under the same limits. The only direct owner-store mutation by Token Jar is the user-approved Grok OIDC renewal described below. The Core data-protection Keychain for app-owned credentials remains a separate fail-closed facility; ad-hoc distribution does not promise it works, and it is not a fallback for Claude or any other owner session.
...
## Claude OAuth usage and bounded automatic recovery

Token Jar accepts only `claudeAiOauth` from the exact native file or generic-password service `Claude Code-credentials` item with `kSecAttrAccount=NSUserName()`. Rejected-token memory is retained independently for the file and Keychain slots (at most two) even when the other store supplies a fresh token, preventing A/B oscillation. MCP tokens are never accepted or renewed. A usable token requires `user:profile`, finite positive expiry, and more than 60 seconds remaining. Expiry is checked after each potentially slow read.

Promotional reset tickets use one additional optional same-token `GET https://api.anthropic.com/api/oauth/usage?cedar_ember=1&skip_spend=1` (15-second maximum). The ordinary usage request remains unchanged. The exact query and `cedar_ember.grants[]` contract are evidenced by the published native `@anthropic-ai/claude-code-darwin-arm64@2.1.280` package, whose npm SHA-512 integrity was verified: `ctkNgja8Yi2kngVFPO2667k6zbtJwjQ+dOTeEp1XmzHcoDFdaee4h4WVgZllsewm/Io+pPPPSFQVdGHOtdE/1A==`. Each grant supplies `id`, `resets_left`, and optional ISO `starts_at` / `ends_at`. The UI sums remaining counts within those dates and shows the nearest remaining expiration using the shared Codex ticket view. `paused`, `usable_now`, and `eligible` do not imply zero owned tickets. Missing or malformed optional metadata stays unknown; a valid empty grant array means zero. Supplemental failures never discard valid primary usage or trigger authentication recovery. Cancellation still propagates. No reset-redemption POST is permitted. Live account validation of this new query is pending: the noninteractive local probe had no usable session and Keychain returned `-25293`; no authenticated HTTP response was obtained.

The fixed OAuth User-Agent and Core header allowlist are aligned to the evidenced `claude-code/2.1.280` version; grant eligibility can explicitly depend on `cli_version`. Local verification: all 218 package tests passed, and all 39 app tests passed with the isolated UITest host under `-testLanguage en -testRegion US`. The initial app run exposed English-only assumptions in an existing Codex test and an incorrect 12-hour expectation in the new expiration test; the latter now matches the shared formatter's explicit 24-hour template. The focused Claude bilingual and Codex ticket XCUITests compiled but could not run: the runner was killed before establishing its test connection, both unsigned and ad-hoc signed. This is not a UI-test pass or live-account proof. Provider I/O audit, 91 bilingual localization keys, source-manifest JSON, and `git diff --check` passed.

Every Claude collection—including startup, restart, the default five-minute schedule, and ordinary manual refresh—first rereads the exact file noninteractively, then the exact Keychain account when the file supplies no usable session. It adopts a changed valid external token. Only a successful live usage response is fresh. Near expiry (60 seconds or less) or usage HTTP 401 can consume that collection's single recovery opportunity; the parent retries usage at most once with a changed valid token. HTTP 403 does not repair or renew. Network, timeout, schema, authorization, 403, 429, or failed recovery retains a prior in-memory result only as stale; there is no stale-cache-as-fresh or `~/.claude.json` fallback. Unavailable Keychain access and a session still needing recovery expose `RecoveryAction.repairClaudeConnection`; a missing/revoked login requires source-owner sign-in.

All Claude collection contexts set `allowsClaudeRecovery=true`; `isUserInitiated` remains truthful about whether the collection was manually started and is not used as recovery authority. Each collection may attempt recovery at most once, so a failure cannot create a tight retry loop. The default schedule remains five minutes. A denied or failed attempt ends that collection, while the next scheduled collection receives its own single opportunity. The localized **Repair Claude Connection / Claude 연결 복구** action remains available as an immediate manual retry under the same limits, and concurrent manual repair callers coalesce.

A recovery keeps the exact account and all ordinary pre-recovery reads noninteractive. Native Keychain authorization is attempted at most once, only after the screen-unlocked guard passes; Token Jar cannot bypass macOS authentication. Denial, cancellation, a locked screen, or authorization failure aborts the current attempt without launching or relaunching an owner CLI and without another prompt loop. If credentials are expired or rejected after rereading, the collection may launch at most one bounded Claude Code owner process. Claude Code may display its own permission UI during that bounded recovery. The process uses fixed absolute executable candidates, an empty mode-0700 workspace, a minimal fixed environment, fixed safe-mode/no-tools arguments, and the existing bounded owner timeout. Prompt veto precedes readiness and success detection; only the exact trust question may be accepted in the verified-empty workspace, and never after `/usage`.

After readiness banner plus shift+tab the owner sends exactly `/usage`; loading or tab headers do not complete, and a recognized completed panel must remain quiet for at least one second. Completion sends Esc and `/exit` and allows five seconds for natural exit. A hard deadline after `/usage` performs a passive five-second wait with no input before bounded termination; a pre-readiness deadline performs immediate cleanup without input. Token Jar never sends a refresh POST or writes, copies, or caches a Claude token. It does not accept MCP, use browser cookies, automate browser/login approval, scrape usage from PTY output, or call an Admin or inference endpoint. Missing or revoked credentials require `claude auth login`; web or Claude Desktop login is not a substitute.

The app displays a spinner and localized collection/manual-repair progress plus native Keychain approval guidance. Manual repair publishes its pending state immediately, including while queued behind a collection, and suppresses duplicate clicks until completion. Last-good quota data remains visible. The UI does not infer internal credential-reread, native-approval, or owner-refresh phases.

The delegated-coordinator policy reference is pinned to [CodexBar `5e119a9ad76e3453058dab323f98e480f478d049`](https://github.com/steipete/CodexBar/commit/5e119a9ad76e3453058dab323f98e480f478d049); the read-only comparison is [OpenUsage `56378e5765f85d38ff413036fd984afe3d4664e4`](https://github.com/robinebers/openusage/commit/56378e5765f85d38ff413036fd984afe3d4664e4). These are design references, not runtime dependencies or proof of Token Jar behavior.

The 2026-09-16 package, app, bilingual UI, audit, catalog, release-tool, and unsigned universal build results verified the now-superseded explicit-repair-only policy at that revision. They do not establish success for the current automatic per-collection recovery policy. Real Keychain authorization, authenticated token rotation, and live-token behavior for this change remain pending; no installation or deployment success is claimed.

2026-09-18 final local QA: all 212 package tests in 12 suites passed three consecutive runs with `swift test --package-path Packages/TokenTankCore -Xswiftc -load-plugin-library -Xswiftc /Library/Developer/CommandLineTools/usr/lib/swift/host/plugins/testing/libTestingMacros.dylib`. The CLT host needs the explicit installed macro plugin; earlier plain incremental/native-engine attempts failed in the toolchain. Release-tool tests (13), Provider I/O audit, 91 bilingual keys, JSON/plist/project-version validation, Swift parsing (including UITEST source), and CI YAML validation passed. The new fixture initially had an async-context semaphore compile error; dispatching fixture cleanup outside that context corrected it before the final three runs. A changed-source credential-pattern scan passed, but gitleaks is absent and a full gitleaks/history audit was not performed. Legacy Keychain deprecation and CLT linker-search warnings remain.

QA rejected the initial candidate's interactive process-policy escalation because a blocked Security call could retain it after caller timeout. The final interactive path never mutates that policy: an existing disabled policy returns `claude.session.keychain.interaction-policy-disabled` before native work. Tests cover that refusal plus timeout/cancellation with the real policy wrapper and blocked synthetic lookups. Source re-review found the introduced HIGH resolved and no new blocking findings. The preexisting native-call lifetime limitation remains: timeout/cancellation bounds the waiter, not `SecItemCopyMatching` or its dialog; late data is discarded and replacement queries are refused until the worker exits. Read-only suppression also lasts until that worker exits. Do not describe this as guaranteed native-call termination or guaranteed recovery every five minutes.

[v0.1.13 (14)](https://github.com/nahwan-kim/token-jar/releases/tag/v0.1.13) is published as an ad-hoc, unnotarized prerelease. Source commit `7e3d024c38f891f79a7f2b7107c987a77152849b` passed [source CI](https://github.com/nahwan-kim/token-jar/actions/runs/35303351124) on both macOS runners with Xcode app/UI automation excluded by owner request. A separate clean checkout on the master's Xcode 27.0 (27A266a) built the universal x86_64/arm64 Release app. Lab completed packaging using the migrated existing Keychain key; the independently extracted ZIP passed signed code, architecture, entitlements, pinned Sparkle footprint, and leakage audits. Gitleaks 8.30.1 found no leaks in 59 source commits or the extracted app. A five-second startup smoke of that extracted signed app passed with provider collection, updater and login-item autostart disabled; it is not live authentication or upgrade evidence.
...
| AC-03 | Historical 2026-09-16 package 206 tests/12 suites ×3 covered the superseded explicit-repair-only policy. | Verify automatic per-collection recovery plus natural-expiry/real-401 authenticated rotation and final clean release. |
| AC-04 | Historical app 35 tests and two bilingual repair/login UI cases covered the superseded policy. | Verify current progress UI and final signed-candidate bilingual behavior. |
| AC-05 | Historical fake-clock/gated results covered a different recovery-authority revision. | Verify the unchanged five-minute cadence, one opportunity per background/manual collection, no tight retry loop, cancellation, and stop/restart. |
| AC-06 | Historical Coordinator/Adapter/AppModel results covered typed failures and retained last-good usage under the superseded policy. | Verify denial aborts only the current attempt, the next scheduled cycle retries, progress clears, and no authority leaks or prompt loop occurs. |
| AC-07 | Historical synthetic session tests covered at-most-one native authorization and owner launch for explicit repair. | Verify the screen-unlocked guard, bounded automatic recovery, live authenticated changed-token recovery, and real Keychain authorization. |
| AC-08 | Historical exact-account and interaction-flag assertions covered noninteractive ordinary collection and explicit repair. | Verify supported-OS native Keychain behavior with silent first reread and at most one native approval per collection; macOS authentication remains mandatory. |
| AC-09 | Historical source-level explicit-repair tests/build and v4 HTTP 200 evidence are revision-scoped and superseded. | Verify automatic scheduled/manual recovery, natural-expiry, real-401 changed-token recovery, and real Keychain authorization. |
| AC-10 | Historical v4 and clean `489ba51` artifacts predate current terminal/cancellation repairs; all v1-v4 and `489ba51` ZIPs are superseded and nonpublishable. | Rebuild only after current-policy CI passes. |
...
| AC-12 | Historical 2026-09-16 package 206/12 ×3, app 35, bilingual UI 2, audit, manifest/catalog validation, release-tool 13 tests, and universal unsigned Release build passed for the superseded explicit-repair-only revision. | Re-run current automatic-recovery checks, then clean signed-artifact and publication CI gates. |
| AC-13 | Full 171 historically exposed cancellation before waiter registration; moving the check inside the installed cancellation handler sealed that gate for the tested revision. | Current source CI, then clean rebuild/package and publication gates. |
...
| AC-15 | Historical explicit-repair tests, Provider I/O audit, manifest/localization validation, and universal unsigned Release build are superseded for current-policy claims. Intentional legacy Keychain warnings remain. | Current automatic-recovery verification, real Keychain/token-rotation validation, signed/notarized packaging, installation, and clean rebuilt-artifact security. |
